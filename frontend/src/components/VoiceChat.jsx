...
export default function VoiceChat({ socket, roomCode }) {
  ...
  const creatingRef = useRef(new Set());        // socketIds being created
  const pendingSignalsRef = useRef(new Map());  // socketId -> Signal[]

  const iceServers = [
    { urls: "stun:stun.relay.metered.ca:80" },
    { urls: "turn:global.relay.metered.ca:80", username: "5de920dfe821f5bc7448b091", credential: "IjNHz31w8QBo1vZq" },
    { urls: "turn:global.relay.metered.ca:80?transport=tcp", username: "5de920dfe821f5bc7448b091", credential: "IjNHz31w8QBo1vZq" },
    { urls: "turn:global.relay.metered.ca:443", username: "5de920dfe821f5bc7448b091", credential: "IjNHz31w8QBo1vZq" },
    { urls: "turns:global.relay.metered.ca:443?transport=tcp", username: "5de920dfe821f5bc7448b091", credential: "IjNHz31w8QBo1vZq" },
  ];
  ...
  // Join the collab room FIRST (server requires this before voice)
  useEffect(() => {
    if (!socket || !roomCode) return;

    const onRoomJoined = () => setRoomJoined(true);
    const onError = (e) => setError(e?.message || "Room error");
  ...
  useEffect(() => {
    ...
    const onParticipants = () => {}; // optional UI
    const onVoiceError = (e) => setError(e?.message || "Voice error");
  ...
  const joinVoice = async () => {
    if (!socket || !roomCode) return;
    if (!roomJoined) {
      setError("Join the room first (auto-joining… click Join again).");
      socket.emit("joinRoom", roomCode);
      return;
    }
    try {
      setConnecting(true);
      setError(null);
      await ensureLocalStream();           // user gesture unlocks autoplay
  ...
  const leaveVoice = () => {
    ...
    setJoined(false);
    peersRef.current.forEach((_p, sid) => removePeer(sid));
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-6 border border-slate-200 dark:border-slate-700 transition-colors">
      ...
      <div className="mt-3 text-xs text-slate-500 dark:text-slate-400">
        Tip: watch the console for ICE states. If stuck on “checking”, TURN credentials or outbound UDP may be blocked.
      </div>
    </div>
  );
}
